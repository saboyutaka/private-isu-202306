  ...
  class App < Sinatra::Base
      ...
      def config
        @config ||= {
          db: {
            ...
            username: ENV['ISUCONP_DB_USER'] || 'isuconp',
            password: ENV['ISUCONP_DB_PASSWORD'] || 'isuconp',
    ...
    get '/@:account_name' do
      ...
      query = <<~QUERY
        ...
        LIMIT 20
      QUERY
      results = db.xquery(query, user[:id])
      posts = make_posts(results)
    ...
    get '/posts' do
      ...
      query = <<~QUERY
        ...
        LIMIT 20
      QUERY
      results = db.xquery(query, max_created_at.nil? ? nil : Time.iso8601(max_created_at).localtime)
      posts = make_posts(results)
    ...
    get '/posts/:id' do
      query = <<~QUERY
        ...
        LIMIT 20
      QUERY
      results = db.xquery(query, params[:id])
